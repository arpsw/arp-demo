...
- When referencing the Filament routes, aim to use `getUrl()` instead of Laravel `route()`. Instead of `route('filament.admin.resources.users.index')`, use `UserResource::getUrl('index')`. Also, specify the exacy Resource name, instead of `getResource()`.
...
- **Deprecated v3 methods - do NOT use:**
  ...
  - `->label('')` for hidden labels → use `->hiddenLabel()` insteadhttps://filamentphp.com/docs/5.x/advanced/enums